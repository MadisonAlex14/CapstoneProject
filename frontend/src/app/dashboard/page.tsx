        ...
        <div className={styles.dGrid}>
          <div className={styles.card}>
            <div className={styles.dCardHeader}>
              <div>
                <h2 className={styles.dSectionTitle}>Credit Score Overview</h2>
                <p className={styles.dSectionSubtitle}>
                  Your current score, goal, and progress at a glance.
                </p>
              </div>

              <span className={`${styles.dScoreBadge} ${scoreRange.className}`}>
                {scoreRange.label}
              </span>
            </div>

            {currentScore !== null ? (
              <div className={styles.dScoreLayout}>
                <div className={styles.dMeterWrap}>
                  <svg className={styles.dMeter} viewBox="0 0 180 180">
                    <circle
                      className={styles.dMeterTrack}
                      cx="90"
                      cy="90"
                      r="70"
                    />
                    <circle
                      className={`${styles.dMeterProgress} ${scoreRange.className}`}
                      cx="90"
                      cy="90"
                      r="70"
                  ...
                  <div className={styles.dMeterCenter}>
                    <span className={styles.dMeterScore}>{currentScore}</span>
                    <span className={styles.dMeterLabel}>Credit Score</span>
                  </div>
                </div>

                <div className={styles.dScoreDetails}>
                  <div className={styles.dStatCard}>
                    <span className={styles.dStatLabel}>Goal Score</span>
                    <span className={styles.dStatValue}>
                      {goalScore !== null ? goalScore : "Not set"}
                    </span>
                  </div>

                  <div className={styles.dStatCard}>
                    <span className={styles.dStatLabel}>Points to Goal</span>
                    <span className={styles.dStatValue}>
                      {pointsToGoal !== null ? pointsToGoal : "—"}
                    </span>
                  </div>

                  <div className={styles.dStatCard}>
                    <span className={styles.dStatLabel}>Score Range</span>
                    <span className={styles.dStatValue}>{scoreRange.label}</span>
                  </div>

                  {goalScore !== null && (
                    <div className={styles.dGoalProgressWrap}>
                      <div className={styles.dGoalProgressTop}>
                        <span>Goal Progress</span>
                        <span>
                          {currentScore >= goalScore ? "100%" : `${Math.min(
                            Math.round((currentScore / goalScore) * 100),
                            100
                          )}%`}
                        </span>
                      </div>
                      <div className={styles.dGoalProgressBar}>
                        <div
                          ...
                          style={{
                            width: `${
                              currentScore >= goalScore
                                ? 100
                                : Math.min((currentScore / goalScore) * 100, 100)
                            }%`,
                          }}
                        />
                      </div>
              ...
              <div className={styles.dEmptyState}>
                <div className={styles.dEmptyIcon}>💳</div>
                <h3 className={styles.dEmptyTitle}>No credit score yet</h3>
                <p className={styles.dEmptyText}>
                  Add your current score in your profile to unlock your dashboard
                  meter, goal tracking, and personalized insights.
                </p>
                <button
                  type="button"
                  className={styles.dPrimaryButton}
          ...
          <div className={styles.dSideColumn}>
            <div className={styles.card}>
              <h2 className={styles.dSectionTitle}>Personalized Insight</h2>
              <p className={styles.dInsightText}>{insightText}</p>
            </div>

            <div className={styles.card}>
              <h2 className={styles.dSectionTitle}>Recommended Next Steps</h2>
              <div className={styles.dStepsList}>
                {nextSteps.map((step, index) => (
                  <div key={index} className={styles.dStepItem}>
                    <span className={styles.dStepNumber}>{index + 1}</span>
                    <span className={styles.dStepText}>{step}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={styles.card}>
              <h2 className={styles.dSectionTitle}>Friendly Reminder</h2>
              <p className={styles.dSectionSubtitle}>
                Little updates make a big difference. Keep your profile current so
                your dashboard stays smart, helpful, and cute.
              </p>
            </div>
          </div>
        </div>
