...
export default function Page() {
  ...
  const validateRow = (raw: Partial<ImportRow>): ImportRow => {
    ...
    if (!dateVal) {
      errors.push('transaction_date required');
    } else {
      const norm = dateVal.includes('/') ? dateVal.replace(/\//g, '-') : dateVal;
      const parsed = new Date(norm);
      if (Number.isNaN(parsed.getTime())) {
        errors.push('transaction_date invalid');
      }
    }
    ...
    const mccString = raw.mcc?.trim() || '';
    if (!/^[0-9]{4}$/.test(mccString)) {
      errors.push('mcc must be 4-digit');
    } else if (!Object.keys(mccToCategory).includes(String(Number(mccString)))) {
      errors.push('mcc not recognized');
    }
        ...
        {step === 3 && (
          <div style={{ overflowX: 'auto' }}>
            <table className="CardDashboardTable">
              ...
              <tbody>
                {rows.map((row, idx) => (
                  <tr key={idx} style={{ background: row.errors.length ? '#ffeaea' : '#fff' }}>
                    ...
                    <td><button className="CardDetailsButtonSecondary" onClick={() => setRows((prev) => prev.filter((_, i) => i !== idx))}>Remove</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
